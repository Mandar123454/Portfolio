...
export type InternDocItem = {
  ...
  certThumb?: string; // Image fallback for mobile PDF viewing (cert)
  lorImage?: string;  // Letter of Recommendation
  lorThumb?: string;  // Image fallback for mobile PDF viewing (lor)
  expImage?: string;  // Experience Letter
  expThumb?: string;  // Image fallback for mobile PDF viewing (exp)
};
...
export default function InternModal({ items }: { items: InternDocItem[] }) {
  ...
  // Choose the active image and thumb based on the requested doc; fallback to available one
  const active = useMemo(() => {
    if (!current) return { src: undefined as string | undefined, thumb: undefined as string | undefined, kind: doc };
    const wanted = doc === "lor" 
      ? { src: current.lorImage, thumb: current.lorThumb } 
      : doc === "exp" 
        ? { src: current.expImage, thumb: current.expThumb } 
        : { src: current.certImage, thumb: current.certThumb };
    if (wanted.src) return { src: wanted.src, thumb: wanted.thumb, kind: doc };
    // Fallback if requested doc missing
    const fallbackOrder: Array<{ src?: string; thumb?: string; kind: "cert" | "lor" | "exp" }> = [
      { src: current.certImage, thumb: current.certThumb, kind: "cert" },
      { src: current.lorImage, thumb: current.lorThumb, kind: "lor" },
      { src: current.expImage, thumb: current.expThumb, kind: "exp" },
    ];
    const alt = fallbackOrder.find((x) => Boolean(x.src));
    return { src: alt?.src, thumb: alt?.thumb, kind: alt?.kind ?? doc };
  }, [current, doc]);
  ...
  const displaySrc = (isMobile && isPdf && active.thumb) ? active.thumb : (!isPdf ? active.src : null);
  useEffect(() => {
  ...
  const showThumbOnMobile = isMobile && isPdf && active.thumb;
              ...
              isPdf ? (
                showThumbOnMobile ? (
                  ...
                  <div className="relative h-full w-full overflow-auto touch-pan-x touch-pan-y touch-pinch-zoom">
                    <Image 
                      src={active.thumb as string} 
                      alt={`${current.title}${titleSuffix}`} 
                      fill 
                      className="rounded-b-[14px] object-contain" 
                      priority 
                      sizes="(max-width: 768px) 96vw, 1200px"
                      unoptimized={active.thumb?.includes('%20') || active.thumb?.includes(' ')}
                    />
                  </div>
                ) : isMobile && !active.thumb ? (
                ...
                <div className="relative h-full w-full overflow-auto touch-pan-x touch-pan-y touch-pinch-zoom">
                  <Image 
                    ...
                    priority 
                    sizes="(max-width: 768px) 96vw, 1200px"
                    unoptimized={active.src?.includes('%20') || active.src?.includes(' ')}
                  />
                </div>
              )
            ) : null}
