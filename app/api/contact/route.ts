...
export async function POST(req: NextRequest) {
  try {
    ...
    const payload = { name, email, phone, message, ip, ua: req.headers.get("user-agent") || "", ts: new Date().toISOString() };
  ...
  const from = ((process.env.CONTACT_FROM_EMAIL || user) as string).trim();
        ...
        const html = `
          <div style="font-family:ui-sans-serif,system-ui,-apple-system,Segoe UI,Roboto,Arial">
            <h2 style="margin:0 0 8px;color:#111">New portfolio contact</h2>
