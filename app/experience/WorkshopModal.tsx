...
export default function WorkshopModal({ items }: { items: WorkshopProofItem[] }) {
  const router = useRouter();
  const params = useSearchParams();
  const view = params.get("workshop");
  ...
  useEffect(() => {
    if (!hasImage || !current?.image) return;
    let isActive = true;
          ...
          <div className="relative h-full w-full rounded-[1rem] bg-black/65 backdrop-blur-sm overflow-hidden">
            ...
            {hasImage ? (
              <Image
                ...
                alt={`${current.title} — ${current.provider}`}
                fill
              ...
              <div className="flex h-full w-full items-center justify-center p-8 text-center">
                <div>
                  <p className="text-base font-semibold text-white">Certificate image not set</p>
                  <p className="mt-2 text-sm text-white/70">
                    Add the certificate image path for this workshop in <span className="text-white">/public</span>, then update the workshop item.
                  </p>
                </div>
              </div>
            )}
